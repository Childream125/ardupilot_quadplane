...
MyToshibaLED_I2C::MyToshibaLED_I2C(uint8_t bus)
    : MyRGBLed(TOSHIBA_LED_OFF, TOSHIBA_LED_BRIGHT, TOSHIBA_LED_MEDIUM, TOSHIBA_LED_DIM)
    , _bus(bus)
{
}

bool MyToshibaLED_I2C::hw_init(void)
{
    ...
    if (ret) {
        _dev->register_periodic_callback(20000, FUNCTOR_BIND_MEMBER(&MyToshibaLED_I2C::_timer, void));
    }
...
bool MyToshibaLED_I2C::hw_set_rgb(uint8_t red, uint8_t green, uint8_t blue)
{
    rgb = {red, green, blue};
    _need_update = true;
    return true;
...
void MyToshibaLED_I2C::_timer(void)
{
    if (!_need_update) {
        return;
    }
