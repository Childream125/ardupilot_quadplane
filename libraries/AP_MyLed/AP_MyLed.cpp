...
#include <stdio.h>
#include "AP_MyLed.h"
#include "MyToshibaLED_I2C.h"

extern const AP_HAL::HAL& hal;

AP_MyLed *AP_MyLed::_singleton;
...
const AP_Param::GroupInfo AP_MyLed::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO("LED_BRIGHT", 0, AP_MyLed, _rgb_led_brightness, RGB_LED_HIGH),
    ...
    // @User: Advanced
    AP_GROUPINFO("BUZZ_ENABLE", 1, AP_MyLed, _buzzer_enable, BUZZER_ENABLE_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("LED_OVERRIDE", 2, AP_MyLed, _rgb_led_override, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("DISPLAY_TYPE", 3, AP_MyLed, _display_type, 0),
    ...
    // @User: Advanced
    AP_GROUPINFO("OREO_THEME", 4, AP_MyLed, _oreo_theme, 0),
#endif
    ...
    // @Values: 0:Disabled
    // @User: Advanced
    AP_GROUPINFO("BUZZ_PIN", 5, AP_MyLed, _buzzer_pin, 0),
#endif
    ...
    // @User: Advanced
    AP_GROUPINFO("LED_TYPES", 6, AP_MyLed, _led_type, BUILD_DEFAULT_LED_TYPE),
    ...
    // @User: Advanced
    AP_GROUPINFO("BUZZ_ON_LVL", 7, AP_MyLed, _buzzer_level, 1),
#endif
    ...
    // @Range: 0 100
    // @Units: %
    AP_GROUPINFO("BUZZ_VOLUME", 8, AP_MyLed, _buzzer_volume, 100),
    
    AP_GROUPINFO("MyLED_DEATIME", 9, AP_MyLed, _led_deatime, 1000),
...
AP_MyLed::AP_MyLed()
{
    ...
    if (_singleton != nullptr) {
        AP_HAL::panic("AP_MyLed must be singleton");
    }
    _singleton = this;
}

// static flags, to allow for direct class update from device drivers
struct AP_MyLed::notify_flags_and_values_type AP_MyLed::flags;
struct AP_MyLed::notify_events_type AP_MyLed::events;

LEDDevice *AP_MyLed::_devices[CONFIG_NOTIFY_DEVICES_MAX];
uint8_t AP_MyLed::_num_devices;

void AP_MyLed::add_backend_helper(LEDDevice *backend)
{
...
void AP_MyLed::add_backends(void)
{
    if (_num_devices != 0) {
        return;
    }
    ...
    for (uint32_t i = 1; i < Notify_LED_MAX; i = i << 1) {
        switch(_led_type & i) {
            ...
            case Notify_LED_ToshibaLED_I2C_Internal:
                ADD_BACKEND(new MyToshibaLED_I2C(TOSHIBA_LED_I2C_BUS_INTERNAL));
                break;
        }
    }
}
...
// initialisation
void AP_MyLed::init(void)
{
    //hal.uartE->printf("AP_MyLed init\r\n");
    // clear all flags and events
    memset(&AP_MyLed::flags, 0, sizeof(AP_MyLed::flags));
    memset(&AP_MyLed::events, 0, sizeof(AP_MyLed::events));
...
void AP_MyLed::update(void)
{
    //会进入到这个update
    //hal.uartE->printf("AP_MyLed update\r\n");
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
            //hal.uartE->printf("device update\r\n");
            _devices[i]->update();
        }
    }
...
void AP_MyLed::handle_led_control(const mavlink_message_t &msg)
{
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
...
void AP_MyLed::handle_play_tune(const mavlink_message_t &msg)
{
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
...
void AP_MyLed::play_tune(const char *tune)
{
    for (uint8_t i = 0; i < _num_devices; i++) {
        if (_devices[i] != nullptr) {
...
void AP_MyLed::set_flight_mode_str(const char *str)
{
    strncpy(_flight_mode_str, str, 4);
    _flight_mode_str[sizeof(_flight_mode_str)-1] = 0;
}

void AP_MyLed::send_text(const char *str)
{
